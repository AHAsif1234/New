...
void AddNewStudent()
{
    ...
    while(!IsValidName)
    {
        ...
        if(strlen(Name) > 30)
        {
            printf(" Error: Name can not be more than 50 characters.\n\n");
            IsValidName = 0;
        }
        if(strlen(Name) <= 0)
        {
    ...
    while(!IsValidBatch)
    {
        ...
        if(strlen(Batch) > 30)
        {
            printf(" Error: Batch can not be more than 10 characters.\n\n");
            IsValidBatch = 0;
        }
        else if(strlen(Batch) <= 0)
        {
            printf(" Error: Batch can not be empty.\n\n");
            IsValidBatch = 0;
        }
        else
        {
    ...
    while(!IsValidNumberOfCourse)
    {
        ...
        if(NumberOfCourses <= 0 || NumberOfCourses > 8)
        {
            printf(" Error: Number of courses can not be more than 8 and lees than 1.\n\n");
    ...
    strcpy(Students[TotalStudents].Batch,Batch);
...
{
    printf("|==========|==============================|==============================|====================|=============|\n");
    printf("|    ID    |        Name                  |            Batch             |       Phone        |  NO.Course  |\n");
    printf("|==========|==============================|==============================|====================|=============|\n");
    ...
    for(i=0; i<TotalStudents; i++)
    {
        ...
        for(j=0; j < (30-strlen(Students[i].Name)); j++)
        {
            printf(" ");
        }
        printf("|");
        printf("%s",Students[i].Batch);
        for(j=0; j < (30-strlen(Students[i].Batch)); j++)
        {
            printf(" ");
        }
        printf("|");
...
void EditStudent(int StudentFoundIndex)
{
    ...
    while(!IsValidBatch)
    {
        printf(" Enter The New Batch(0 for skip): ");
        scanf("%s",&NewBatch);

        if(strlen(NewBatch) > 30)
        {
            printf(" Error: Batch can not be more than 30 characters.\n\n");
            IsValidBatch = 0;
        }
        else if(strlen(NewBatch) <= 0)
        {
            printf(" Error: Batch can not be empty.\n\n");
            IsValidBatch = 0;
        }
        else if(IsAlreadyExists(NewBatch,'e',StudentID) > 0)
        {
            printf(" Error: This Batch Already Exists.\n\n");
            IsValidBatch = 0;
        }
        else
        {
    ...
    while(!IsValidNumberOfCourse)
    {
        ...
        if(NewNumberOfCourses > 8 || NewNumberOfCourses < 0)
        {
    ...
    if(strcmp(NewBatch,"0") != 0)
    {
        strcpy(Students[StudentFoundIndex].Batch,NewBatch);
    }
...
int IsAlreadyExists(char GivenLine[300],char InfoType, char StudentID[300])
{
    ...
    for(ep=0; ep<TotalStudents; ep++)
    {
        ...
        if(strcmp(GivenLine,Students[ep].Batch) == 0 && strcmp(StudentID,Students[ep].ID) != 0 )
        {
            BatchExists++;
        }
...
void DataSeed()
{
    ...
    //-- student 1
    strcpy(Students[0].ID,"213031001");
    strcpy(Students[0].Name,"MD. SAZZAD AMIN");
    strcpy(Students[0].Phone,"01818518044");
    strcpy(Students[0].Batch,"26th");
    Students[0].NumberOfCourse=1;

    strcpy(Courses[0].StudentID,"213031001");
    strcpy(Courses[0].Code,"CSE-311");
    strcpy(Courses[0].Name,"System Analysis & Design");

    //-- student 2
    strcpy(Students[1].ID,"212031006");
    strcpy(Students[1].Name,"SABRINA TABASSUM");
    strcpy(Students[1].Phone,"01980013714");
    strcpy(Students[1].Batch,"25th");
    Students[1].NumberOfCourse=2;

    strcpy(Courses[1].StudentID,"212031006");
    strcpy(Courses[1].Code,"CSE-421");
    strcpy(Courses[1].Name,"Microprocessor");

    strcpy(Courses[2].StudentID,"212031006");
    strcpy(Courses[2].Code,"CSE-311");
    strcpy(Courses[2].Name,"System Analysis & Design");
